...
const ListExample: React.FC = () => {
  ...
  const renderListItems = () => {
    return data.map((item) => (
        <div>
      <li key={item.id} className={styles.for_li_ui}>
        <div>{item.success}</div>
        <div>{item.name}</div>
        <div>{item.failed}</div>
      </li>
      <div className={styles.gaugeBarContainer}>
          <div className={styles.successGaugeBar} style={{ width: `${(item.success / (item.success + item.failed)) * 100}%` }}></div>
          <div className={styles.failedGaugeBar} style={{ width: `${(item.failed / (item.success + item.failed)) * 100}%` }}></div>
        </div>
      </div>
    ));
  };
